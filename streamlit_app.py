...
import streamlit as st, pandas as pd, plotly.express as px, time, datetime as dt
import gspread
...
df = load_sheet()
if df.empty:
    st.warning("В таблице пока нет строк.")
    st.stop()


st.sidebar.header("Фильтры")
users   = st.sidebar.multiselect("Пользователь", sorted(df["Пользователь"].unique()))
methods = st.sidebar.multiselect("Алгоритм",     sorted(df["Алгоритм"].unique()))
...
st.subheader("Данные")
st.download_button("💾 Скачать CSV", df.to_csv(index=False).encode(),
                   "human_study_results.csv", "text/csv")
